...
int main(int argc, char **argv)
{
	...
	socklen_t dimaddr = sizeof(sockfds);
	while(1)
	{
		char filename[1024] = {};
		char command[1024] = {};
	...
	/*apro il file da mandare*/
			...
			printf("dim file %zu\n",fsize);
			bytes_sent = sendto(sockfds, buff,sizeof(buff),0,(struct sockaddr*)&addr,sizeof(addr));/*invio la dim del file*/
			...
			rc = sendto(sockfds,buff,strlen(buff), 0,(struct sockaddr*)&addr,sizeof(addr));
			if(rc == -1)	
		...
		{   
			...
			if(controllo == -1)
				funz_error("Errore lettura file");
			rc = sendto(sockfds,content,strlen(content), 0,(struct sockaddr*)&addr,sizeof(addr));
			if(rc == -1)	
				funz_error("Errore durante l'invio del file\n");
			if(rc != stat_buf.st_size)
				funz_error("Trasferimento incompleto\n");
